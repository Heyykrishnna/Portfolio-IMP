...
/** Safe display name for MIME From header (no HTML). */
function mailDisplayName(s: string): string {
  return s.replace(/[\r\n"]/g, '').trim().slice(0, 200) || 'Contact';
}

const OWNER_INBOX = 'khandelwalyatharth39@gmail.com';

export async function sendContactEmail(
  env: NodeJS.ProcessEnv,
  body: ContactEmailPayload,
): Promise<{ ok: true } | { ok: false; status: number; error: string }> {
  const { name, email, budget, message, services, source } = body;
  ...
  if (!emailUser || !emailPass) {
    return {
      ok: false,
      status: 500,
      error: 'Email is not configured (EMAIL_USER / EMAIL_PASS).',
    };
  }
  ...
  const fromDisplay = mailDisplayName(String(name));
  const safeName = escapeHtml(String(name));
  const safeEmail = escapeHtml(String(email));
  const safeBudget = escapeHtml(String(budget || 'Not specified'));
  const safeServices = escapeHtml(String(services || 'Not specified'));
  const safeSource = escapeHtml(String(source || 'General Contact'));
  const safeMessage = escapeHtml(String(message)).replace(/\n/g, '<br>');

  try {
    await transporter.sendMail({
      from: `"${fromDisplay}" <${emailUser}>`,
      to: OWNER_INBOX,
...
font-family:'Doto','DotGothic16','Segoe UI',Arial,sans-serif;">
  ...
  border:1px solid #362f24;">
    ...
    return { ok: false, status: 500, error: 'Failed to send emails' };
  }
}
